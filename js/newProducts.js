...
function  getNewProductFromDenim(URL){
    if(localStorage.getItem('userNumberOfOrder') !== null){
                   ...
                   // priceAfterDiscountArr.push((Number.parseInt(data.first[item].price) ) - (Number.parseInt(data.first[item].price) * data.first[item].discount / 100));
                    ...
                    quickView.addEventListener("click",function(){
                        ...
                        if(document.body.getAttribute("language-version") == "arabic"){
                            ...
                            listItem1.style.cssText = `background:#2BDE73;color: #FFFFFF;text-align: center;`;
                       ...
                       cardSection2.append(cardSection2_headers,cardSection2_Sizes);
                        ...
                        if(header.innerHTML.split(":")[1] !== undefined){
                            ...
                            cartShop.title = localStorage.getItem("userNumberOfOrder");
                            let orderDetails = {"imageURL":data.first[item].imageURL,"price_beforDiscount":data.first[item].price,"price_afterDiscount":(Number.parseInt(data.first[item].price) ) - (Number.parseInt(data.first[item].price) * data.first[item].discount / 100) + "EGP","size":header.innerHTML.split(":")[1],"quantity":ProductNumberOfOrder};
                                if (performance.navigation.type == performance.navigation.TYPE_RELOAD) {
...
getNewProductFromDenim("../json/clothing/denim.json");
getNewProductFromDenim("../json/clothing/jakets.json");
getNewProductFromDenim("../json/clothing/pants.json");
getNewProductFromDenim("../json/clothing/shirts.json");
getNewProductFromDenim("../json/clothing/sweat_shirt.json");
getNewProductFromDenim("../json/shoes/boats.json");
getNewProductFromDenim("../json/shoes/sandals.json");
getNewProductFromDenim("../json/shoes/sneakers.json");
