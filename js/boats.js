...
if (localStorage.getItem("orderDeatils") != null) {
  for (
    let i = 0;
    i < JSON.parse(localStorage.getItem("orderDeatils")).length;
    i++
  ) {
    cartDetailsBackupArr[i] = JSON.parse(localStorage.getItem("orderDeatils"))[
      i
    ];
  }
}
if (localStorage.getItem("favorite") != null) {
  for (
    let i = 0;
    i < JSON.parse(localStorage.getItem("favorite")).length;
    i++
  ) {
    ...
    if (request.readyState == 4 && request.status == 200) {
      ...
      for (let item in data.first) {
        ...
        if (data.first[item].new_product == true) {
          ...
          if (localStorage.getItem("app_version") == "arabic") {
            newProductDadge.classList.add(
              "new-product-badge-arabic-orientation"
            );
            newProductDadge.innerHTML = "جديد";
          } else {
            newProductDadge.classList.remove(
              "new-product-badge-arabic-orientation"
            );
        ...
        if (localStorage.getItem("app_version") == "arabic") {
          favoriteProductContainer.classList.add(
            "favorite-product-container-arabic-orientation"
          );
        }
        ...
        favoriteProductContainer.addEventListener("click", () => {
          ...
          if (favoriteProductContainer.classList.contains("favorite")) {
            ...
            favoritSuccessfulAlert.innerHTML =
            ...
            for (
              ...
              if (
                JSON.parse(localStorage.getItem("favorite"))[i].imageURL ==
                data.first[item].imageURL
              ) {
        ...
        if (localStorage.getItem("app_version") == "arabic") {
          price_befor_discount.innerHTML =
            data.first[item].price.split("EGP")[0] + "" + "ج";
        } else {
        ...
        if (data.first[item].discount > 0) {
          ...
          if (localStorage.getItem("app_version") == "arabic") {
            price_after_discount.innerHTML =
              Number.parseInt(data.first[item].price) -
              (Number.parseInt(data.first[item].price) *
                data.first[item].discount) /
                100 +
              "ج";
          } else {
            price_after_discount.innerHTML =
          ...
          price_befor_discount.style.cssText =
            "text-decoration: line-through; color:#040000;";
          productPriceContainer.appendChild(price_after_discount);
          let discountBadge = document.createElement("div");
          discountBadge.className = "discount-badge";
          if (localStorage.getItem("app_version") == "arabic") {
            discountBadge.innerHTML =
              "خصم" + " " + data.first[item].discount + "%";
          } else {
            discountBadge.innerHTML =
              "Save" + " " + data.first[item].discount + "%";
          }
          ...
          if (data.first[item].stock == "InStock") {
            ...
            for (let i = 0; i < data.first[item].size.length; i++) {
              ...
              size_number.addEventListener("click", function (e) {
                ...
                /* arabic version */
                if (
                  document.body.getAttribute("language-version") == "arabic"
                ) {
                  header.innerHTML =
                    "مقاس:" + " " + data.first[item].size[i].value;
                } else {
                  header.innerHTML =
                    "Size:" + " " + data.first[item].size[i].value;
                }
            ...
            let cardSection3_row1_col2_row1_col1 =
              document.createElement("span");
            /* arabic version */
            if (document.body.getAttribute("language-version") == "arabic") {
              cardSection3_row1_col2_row1_col1.innerHTML =
                Number.parseInt(data.first[item].price) -
                (Number.parseInt(data.first[item].price) *
                  data.first[item].discount) /
                  100 +
            ...
            let cardSection3_row1_col2_row1_col2 =
              document.createElement("span");
            /* arabic version */
            if (document.body.getAttribute("language-version") == "arabic") {
              cardSection3_row1_col2_row1_col2.innerHTML =
                data.first[item].price.split("EGP")[0] + "ج";
            } else {
              cardSection3_row1_col2_row1_col2.innerHTML =
                data.first[item].price;
            }

            let cardSection3_row1_col2_row2 = document.createElement("span");
            /* araabic version */
            if (document.body.getAttribute("language-version") == "arabic") {
              cardSection3_row1_col2_row2.innerHTML =
                "شامل الضرائب. شحن مجاني لجميع الطلبات";
            } else {
              cardSection3_row1_col2_row2.innerHTML =
                "Taxes are included. Free shipping on all orders.";
            }
            if (
              Number.parseFloat(cardSection3_row1_col2_row1_col2.innerHTML) !==
              Number.parseFloat(cardSection3_row1_col2_row1_col1.innerHTML)
            ) {
            ...
            cardSection3_row1.append(
              cardSection3_row1_col1,
              cardSection3_row1_col2,
              cardSection3_row1_col2_row2
            );
            ...
            quantitySettings.append(
              quantitySettings_li1,
              quantitySettings_li2,
              quantitySettings_li3
            );
            ...
            addToCartButton.addEventListener("click", function () {
              if (header.innerHTML.split(":")[1] !== undefined) {
                if (SoldOut == false) {
                  ...
                  cartShop.title = localStorage.getItem("userNumberOfOrder");
                  let orderDetails = {
                    imageURL: data.first[item].imageURL,
                    price_beforDiscount: data.first[item].price,
                    size: header.innerHTML.split(":")[1],
                    quantity: ProductNumberOfOrder,
                  };
                  if (data.first[item].discount > 0) {
                    orderDetails["price_afterDiscount"] =
                      Number.parseInt(data.first[item].price) -
                      (Number.parseInt(data.first[item].price) *
                        data.first[item].discount) /
                        100 +
  ...
  if (document.body.contains(ourCartShop) == false) {
    ...
    */
    ...
    checkoutBtn.addEventListener("click", function () {
      if (
        localStorage.getItem("userNumberOfOrder") !== 0 &&
    ...
    function getTotalPriceAndTotalSavings() {
      ...
      for (
        let j = 0;
        j < JSON.parse(localStorage.getItem("orderDeatils")).length;
        j++
      ) {
        /*
    ...
    */
        if (
          JSON.parse(localStorage.getItem("orderDeatils"))[j]
            .price_afterDiscount != undefined
        ) {
          // totalPriceAfterDiscount +=  Number.parseFloat(JSON.parse(localStorage.getItem("orderDeatils"))[j].price_afterDiscount.split("EGP")[0]);
          totalPriceSavedAfterDiscount +=
            Number.parseFloat(
              JSON.parse(localStorage.getItem("orderDeatils"))[
                j
              ].price_beforDiscount.split("EGP")[0]
            ) -
            Number.parseFloat(
              JSON.parse(localStorage.getItem("orderDeatils"))[
                j
              ].price_afterDiscount.split("EGP")[0]
            );
        }
        if (
          JSON.parse(localStorage.getItem("orderDeatils"))[j]
      ...
      if (localStorage.getItem("app_version") == "arabic") {
        ...
        span3.innerHTML =
          "لقد ادخرت " + " " + totalPriceSavedAfterDiscount + " " + "ج";
      } else {
        span2.innerHTML = totalPrice + " " + "EGP";
        span3.innerHTML =
          "You Saved:" + " " + totalPriceSavedAfterDiscount + " " + "EGP";
      }
    }
    ...
    for (
      ...
      order_image.src = JSON.parse(localStorage.getItem("orderDeatils"))[
        i
      ].imageURL;
      ...
      if (localStorage.getItem("app_version") == "arabic") {
        order_price_befoeDiscount.innerHTML =
          JSON.parse(localStorage.getItem("orderDeatils"))[
            i
          ].price_beforDiscount.split("EGP")[0] +
          " " +
        ...
        order_price_befoeDiscount.innerHTML = JSON.parse(
          localStorage.getItem("orderDeatils")
        )[i].price_beforDiscount;
      }
      if (
        JSON.parse(localStorage.getItem("orderDeatils"))[i]
          .price_afterDiscount != undefined
      ) {
      ...
      if (
        ...
        if (localStorage.getItem("app_version") == "arabic") {
          order_price_afterDiscount.innerHTML =
            JSON.parse(localStorage.getItem("orderDeatils"))[
              i
            ].price_afterDiscount.split("EGP")[0] +
            " " +
      ...
      if (localStorage.getItem("app_version") == "arabic") {
        order_size.innerHTML =
          "المقاس:" +
          " " +
          JSON.parse(localStorage.getItem("orderDeatils"))[i].size;
      } else {
        order_size.innerHTML =
          "Size:" +
          " " +
      ...
      order_price_size_container.append(
        priceBeforAndAfterContainer,
        order_size
      );
      ...
      quantitySettings_li1.addEventListener("click", function () {
        let QunatityFromLocalStorage = Number(
          JSON.parse(localStorage.getItem("orderDeatils"))[i].quantity
        );
        QunatityFromLocalStorage -= 1;
        let localStorageArrayOfObjects = JSON.parse(
          localStorage.getItem("orderDeatils")
        );
        localStorageArrayOfObjects[i].quantity = QunatityFromLocalStorage;
        localStorage.setItem(
          "orderDeatils",
          JSON.stringify(localStorageArrayOfObjects)
        );
        quantitySettings_li2.innerHTML = JSON.parse(
          localStorage.getItem("orderDeatils")
        )[i].quantity;
      });
      let quantitySettings_li2 = document.createElement("li");
      ...
      removeProductFromCart.addEventListener("click", function () {
        let cartArrFromLocalStorage = JSON.parse(
          localStorage.getItem("orderDeatils")
        );
        ...
        localStorage.setItem(
          "orderDeatils",
          JSON.stringify(cartArrFromLocalStorage)
        );
      ...
      order_details_container.append(
        order_image_container,
        order_price_size_container,
...
let usernameWelcomeSpan = document.querySelector(
  ".header_login_signup_container > span"
);
...
let listItemMenuListItemLink = document.querySelectorAll(
  "ul > li > a+ul > span > li > a"
);
let listItemMenuListItemLink_arabic_arr = [
  "جكتات ومعاطف",
  "جينز",
  "بناطيل",
  "تيشرتات",
...
let listItemMenuListItemLink_english_arr = [
  "Jaket",
  "Denim",
  "pants",
  "Shirts",
...
if (localStorage.getItem("app_version") !== null) {
  ...
  if (localStorage.getItem("app_version") === "english") {
    ...
    if (localStorage.getItem("username") !== null) {
      usernameWelcomeSpan.innerHTML =
        "مرحبا " + localStorage.getItem("username");
    } else {
...
let userStatusSection = document.querySelector(
  ".header_login_signup_container"
);
...
loginBtn.onclick = function () {
  window.location.href =
    window.location.protocol + "//" + window.location.host + "/html/form.html";
};
