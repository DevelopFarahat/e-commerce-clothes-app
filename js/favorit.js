...
if(localStorage.getItem("app_version")=="arabic"){
    ...
    if(localStorage.getItem('favorite') !== null){
    function  getFavoritProducts(){
    let favoritArr = JSON.parse(localStorage.getItem("favorite"));
    for(let item = 0 ; item < favoritArr.length;item++){
        ...
        cardImg.src = favoritArr[item].imageURL;
        card.appendChild(cardImg);
        if(favoritArr[item].new_product == true){
        ...
        favoriteProductContainer.addEventListener('click',(event)=>{
            //favoriteProductContainer.classList.toggle("favorite");
            if(favoriteProductContainer.classList.contains("favorite")){
              //  for(let i = 0 ; i < JSON.parse(localStorage.getItem('favorite')).length ; i++){
                    if(JSON.parse(localStorage.getItem('favorite'))[item].imageURL == cardImg.src){
                       let targetFavProObject = JSON.parse(localStorage.getItem('favorite'));
                       targetFavProObject.splice(item,1);
                        localStorage.setItem('favorite',JSON.stringify(targetFavProObject));

                        favoriteProductContainer.classList.remove("favorite");
                        card.remove();
                        if(JSON.parse(localStorage.getItem("favorite")).length == 0){
                            displayImageIncaseOfEmptyFavList();
        ...
        for(let i = 0 ; i < JSON.parse(localStorage.getItem('favorite')).length ; i++){
        if(favoritArr[item].imageURL == JSON.parse(localStorage.getItem('favorite'))[i].imageURL){
            favoriteProductContainer.classList.add("favorite");
        }
        }
        function displayImageIncaseOfEmptyFavList(){
            let favContainer = document.querySelector(".favorite-container");
            let emptyFavoriteImg = document.createElement("img");
            emptyFavoriteImg.src = "/images/empty-folder.png ";
            emptyFavoriteImg.classList.add("empty-favorite-img");
            favContainer.append(emptyFavoriteImg);
        }
        if(localStorage.getItem("favorite") == null){
            displayImageIncaseOfEmptyFavList();
        }
        ...
        if(localStorage.getItem('app_version') == "arabic"){
            price_befor_discount.innerHTML = favoritArr[item].price.split("EGP")[0] + ""+"ج";
        }else{
            price_befor_discount.innerHTML = favoritArr[item].price;
        }
        if(favoritArr[item].discount > 0 ){
        let price_after_discount = document.createElement("span");
        if(localStorage.getItem('app_version') == "arabic"){
            price_after_discount.innerHTML = (Number.parseInt(favoritArr[item].price) ) - (Number.parseInt(favoritArr[item].price) * favoritArr[item].discount / 100) + "ج";
        }else{
            price_after_discount.innerHTML = (Number.parseInt(favoritArr[item].price) ) - (Number.parseInt(favoritArr[item].price) * favoritArr[item].discount / 100) + "EGP";
        }
       
       // priceAfterDiscountArr.push((Number.parseInt(favoritArr[item].price) ) - (Number.parseInt(favoritArr[item].price) * favoritArr[item].discount / 100));
        ...
        if(localStorage.getItem('app_version') == "arabic"){
            discountBadge.innerHTML = "خصم"+ " " + favoritArr[item].discount + "%";
        }else{
            discountBadge.innerHTML = "Save"+ " " + favoritArr[item].discount + "%";
        }
        
        //discountArr.push(favoritArr[item].discount);
        ...
        quickView.addEventListener('mouseenter',function(){
            if(favoritArr[item].stock == "SoldOut"){
        ...
        if(favoritArr[item].stock == "InStock"){
            if(localStorage.getItem('app_version') == "arabic"){
        ...
        quickView.addEventListener("click",function(){
            ...
            productImg1.src = favoritArr[item].imageURL;
          ...
          //  productImg2.src = data.second[item];
            orderDetailsCol1.append(productImg1);
            let imgGallary = document.createElement("img");
            imgGallary.src = favoritArr[item].imageURL;
           ...
           /* arabic version */
            ...
            if(favoritArr[item].discount !== 0){
               /* araibic version */
                if(document.body.getAttribute("language-version") == "arabic"){
                    listItem2.innerHTML = "خصم"+" "+ favoritArr[item].discount + "%";
                }else{
                    listItem2.innerHTML = favoritArr[item].discount + "%";
                }
            ...
            listItem3.style.cssText = `font-size: 1.5rem;float: right;cursor: pointer;text-align: right;`
            /* arabic version */
            if(document.body.getAttribute("language-version") == "arabic"){
            ...
            for(let i = 0; i < favoritArr[item].size.length; i++){
                let size_number = document.createElement("span");
                size_number.className = "size-number";
                size_number.id = i;
                size_number.innerHTML = favoritArr[item].size[i].value;
                var SoldOut = false;
                if(favoritArr[item].size[i].stcok == "SoldOut"){
                ...
                size_number.addEventListener('click',function(e){
                    ...
                    /* arabic version */
                    if(document.body.getAttribute("language-version") == "arabic"){
                        header.innerHTML = "مقاس:" +  " " +favoritArr[item].size[i].value;
                    }else{
                        header.innerHTML = "Size:" +  " " +favoritArr[item].size[i].value;
                    }
           ...
           cardSection2.append(cardSection2_headers,cardSection2_Sizes);
            ...
            /* arabic version */
            if(document.body.getAttribute("language-version") == "arabic"){
                cardSection3_row1_col2_row1_col1.innerHTML = (Number.parseInt(favoritArr[item].price) ) - (Number.parseInt(favoritArr[item].price) * favoritArr[item].discount / 100) + "ج";
            }else{
                cardSection3_row1_col2_row1_col1.innerHTML = (Number.parseInt(favoritArr[item].price) ) - (Number.parseInt(favoritArr[item].price) * favoritArr[item].discount / 100) + "EGP";
            }
            let cardSection3_row1_col2_row1_col2 = document.createElement("span");
            /* arabic version */
            if(document.body.getAttribute("language-version") == "arabic"){
                cardSection3_row1_col2_row1_col2.innerHTML = favoritArr[item].price.split("EGP")[0] + "ج";
            }else{
                cardSection3_row1_col2_row1_col2.innerHTML = favoritArr[item].price;
            }
            ...
            if(header.innerHTML.split(":")[1] !== undefined){
                ...
                cartShop.title = localStorage.getItem("userNumberOfOrder");
                orderDetails = {"imageURL":favoritArr[item].imageURL,"price_beforDiscount":favoritArr[item].price,"size":header.innerHTML.split(":")[1],"quantity":ProductNumberOfOrder};
                if(favoritArr[item].discount > 0){
                    orderDetails["price_afterDiscount"] = (Number.parseFloat(favoritArr[item].price) ) - (Number.parseFloat(favoritArr[item].price) * favoritArr[item].discount / 100) + "EGP"
                }
